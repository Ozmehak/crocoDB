...
function newCrocodilia(event){
    ...
    let formCrocodile = elformCrocodile.value;
...
async function postData(url = '', data = {}) {
    ...
    const response = await fetch(url, {
        ...
        body: JSON.stringify({speciesName:speciesName, speciesFood:speciesFood, speciesLength:speciesLength, speciesWeight:speciesWeight}) // body data type must match "Content-Type" header
    });
    return response.json(); // parses JSON response into native JavaScript objects
}

postData('http://localhost:3000/species')
    .then((data) => {
